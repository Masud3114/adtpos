	...
	&& strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
			...
			if(!in_array($_POST['item_code'],$_SESSION['item_code'])){
				...
				if(@mysql_num_rows($item_qry)>0){
					...
					$data= '<tr>'.
							...
							'<td>
								<input type="number" step="any" value="1" class="form-control" id="item_qty" name="item_qty[]" >
							</td>'.
							'<td>
							...
							<td>
								<input type="number" required value="'.$item_info['item_srate'].'" class="form-control" id="item_srate" name="item_srate[]" >
							</td>'.
		...
		if(isset($_POST[ac_method]) && $_POST[ac_method]=='add_data'){
			...
			if(!in_array($citem_info['bar_cod'],$_SESSION['item_code'])){
				...
				$item_qry=$cmncls->sqlqry($item_sql);
				if(@mysql_num_rows($item_qry)>0){
					$_SESSION['item_code'][]=$citem_info['bar_cod'];
					$item_info=@mysql_fetch_assoc($item_qry);
					//Buy Rate
					if($item_info['item_brate']==0){
						$item_info['item_brate']=NULL;
					...
					if($_POST['item_srate']==NULL){
						$item_srate=$item_info['item_srate'];
					}else{
						$item_srate=$_POST['item_srate'];
					}
					//echo $item_info['item_code'];
					$data= '<tr>'.
							'<td>'
							...
							<td>
								<input type="number" required value="'.$item_srate.'" class="form-control" id="item_srate" name="item_srate[]" >
							</td>'.
							'<td><button type="button"  data="delete_item" class="btn btn-danger waves-effect delete_row">'.
									'<i class="material-icons">delete</i>'.
								'</button>'.
		...
		if(isset($_POST['ac_method']) && $_POST['ac_method']=='rtrive'){
			if(!isset($_SESSION['purchase_slno'])){
				$_SESSION['purchase_slno']=$_POST['sx_code'];
			}
			$prdct_sql="SELECT inop_details.item_code,
			item_info.bar_cod,item_info.item_name,
			inop_details.item_qty,viunit_info.pram_name as xitem_unit,
			inop_details.buy_rate,inop_details.sale_rate,item_info.item_vatrate
			FROM inop_details 
			...
			if(@mysql_num_rows($prdct_qry)>0){
				...
				WHILE($item_info=@mysql_fetch_array($prdct_qry)){
					...
					$data .= '<tr>'.
							...
							<td>
								<input type="number" required value="'.$item_info['sale_rate'].'" class="form-control" id="item_srate" name="item_srate[]" >
							</td>'.
							'<td><button type="button"  data="delete_item" class="btn btn-danger waves-effect delete_row">'.
									'<i class="material-icons">delete</i>'.
								'</button>'.
		...
		if(isset($_POST['add_trn'])){
			if(!isset($_SESSION['purchase_slno'])){
				...
				VALUES ('".$hslno."','".$_SESSION['zid']."','1','4','".$_POST['dsup_cod']."','".$_POST['trn_date']."','".$_POST['trn_ref']."','".$_POST['trn_description']."','".$_SESSION['user_id']."')";
				$cmncls->sqlqry($rcvh_sql);
			}else{
				$hslno = $_SESSION['purchase_slno'];
				$rcvupdt_data=array(
				...
				if ($chng_sts == true){
					$action->delete('inop_details','hslno',$_SESSION['purchase_slno']);
				}
			}
			$rcv_sql="INSERT INTO inop_details(slno, zid, hslno, item_code, item_qty, buy_rate,sale_rate, vat_rate, ent_id) VALUES ";
			$i=0;
			...
			foreach($_POST['item_code'] as $item_code){
				...
				$rcv_sqli.="('".$slno."','".$_SESSION['zid']."','".$hslno."','".$item_code."','".$_POST['item_qty'][$i]."','".$_POST['item_brate'][$i]."','".$_POST['item_srate'][$i]."','".$_POST['item_vatrate'][$i]."','".$_SESSION['user_id']."'),";
				$i++;
				$slno++;
			}
			if($rcv_sqli!=NULL){
				...
				if(!@mysql_errno()){
					...
					$Acc_sts=$accounts->InopPurchaseToAccounts($data,'add');
					if($Acc_sts===true){
