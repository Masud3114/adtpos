...
&& strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	if(isset($_POST['ac_method'])){
		if($_POST['ac_method']=='addFromScan'){
			...
			if(!isset($_SESSION['pos_slno'])){
				$_SESSION['pos_slno']=$_POST['sx_code'];
			}
		...
		else if($_POST['ac_method']=='pause_pos_trn'){
			if(isset($_SESSION['item_code']) && isset($_POST['markedText'])){
				if(!isset($_SESSION['pause_pos'])){
					$_SESSION['pause_pos']=array();
				}
				$_SESSION['pause_pos'][]=array(
					'ref_text'			=> $_POST['markedText'],
					'item_code'			=> $_SESSION['item_code'],
					'item_row'			=> $_SESSION['item_row']
				);
				$pause_row=max(array_keys($_SESSION['pause_pos']));
				$disp_data='
					<tr>
						<td>'.$_POST['markedText'].'</td>
						<td>'.count($_SESSION['item_code']).'</td>
						<td>
							<button title="Restore" name="puaseToactive" type="button" data="'.$pause_row.'" class="btn btn-info waves-effect puaseToactive">
								<i class="material-icons">restore</i>
							</button>
							<button title="Remove" name="puaseTodelete" type="button" data="'.$pause_row.'" class="btn btn-danger waves-effect puaseTodelete">
								<i class="material-icons">delete</i>
							</button>
						</td>
					</tr>
				...
				$rtn_data=array(
					...
					"sts"			=> "1",
					"title"			=> "Paused!",
					"text"			=> count($_SESSION['item_code'])." Item!",
					"type"			=> "warning"
				);
			}else{
				$rtn_data=array(
					"sts"			=> "2",
					"title"			=> "Warning!",
					"text"			=> "Not Found Any Selected Product!",
					"type"			=> "warning"
				);
			}
			echo json_encode($rtn_data);
		}
		else if($_POST['ac_method']=='puaseToactive'){
			$_SESSION['item_code']=array();
			$_SESSION['item_row']=array();
			$_SESSION['item_code']=$_SESSION['pause_pos'][$_POST['row']]['item_code'];
			$_SESSION['item_row']=$_SESSION['pause_pos'][$_POST['row']]['item_row'];
			//echo json_encode($_SESSION['item_row']);
			//exit;
			$data=NULL;
			if(count($_SESSION['item_code'])>0){
				foreach($_SESSION['item_row'] as $item_row){
					$data.= '<tr>
							<td  class="col-sm-3">'
								.$item_row['item_name'].
								'<input type="hidden" class="item_code" name="item_code[]" value="'.$item_row['item_code'].'" />
							</td>
							<td>
								<span class="stock_balance">'.$item_row['stock_balance'].'</span>
							</td>
							<td class="col-sm-2">
								<input type="number" value="'.$item_row['item_qty'].'" class="form-control item_line_qty" name="item_qty[]" >
							</td>
							<td>
								<span>'.$item_row['xitem_unit'].'</span>
							</td>
							<td>
								<span>'.$item_row['item_srate'].'</span>
							</td>
							<td>
								<span class="item_amount">'.$item_row['item_srate'].'</span>
							</td>
							<td>
								<span class="discount_amnt">'.$item_row['discount_amnt'].'</span>
							</td>
							<td>
								<span class="net_amount">'.$item_row['net_amount'].'</span>
							</td>
							<td>
								<button type="button" class="btn btn-danger waves-effect delete_row">
									<i class="material-icons">delete</i>
			...
			unset($_SESSION['pause_pos'][$_POST['row']]);
			echo json_encode($rtn_data);
		}
		else if($_POST['ac_method']=='puaseTodelete'){
			unset($_SESSION['pause_pos'][$_POST['row']]);
			echo json_encode(['sts'	=>'1']);
		}
		else if($_POST['ac_method']=='total_calculate'){
			if($_SESSION['item_row'] && count($_SESSION['item_row'])>0){
			...
			if($main_key!==NULL && $stock_sts==='ok'){
				...
				$_SESSION['item_row'][$main_key]['sub_total_amnt']=round(($_POST['tr_qty']*$item_srate),2);
				$_SESSION['item_row'][$main_key]['discount_amnt']=round((($item_discount/100)*$item_srate)*$_POST['tr_qty'],2);
				$_SESSION['item_row'][$main_key]['net_amount']=round(($_SESSION['item_row'][$main_key]['sub_total_amnt']-$_SESSION['item_row'][$main_key]['discount_amnt']),2);
				$_SESSION['item_row'][$main_key]['vat_amount']=round((($item_vatrate/100)*$_SESSION['item_row'][$main_key]['net_amount']),2);
				$_SESSION['item_row'][$main_key]['net_pay']=round(($_SESSION['item_row'][$main_key]['net_amount']+$_SESSION['item_row'][$main_key]['vat_amount']),2);
			...
			if(!isset($_SESSION['pos_slno'])){
				...
				if($_POST[trn_cash]>$_POST[trn_net_pay]){
					$_POST['trn_cash']=$_POST['trn_net_pay'];
				}
				$shead_sql="INSERT INTO inop_header(slno, zid, trn_type, trn_cat, trn_to, trn_mobile_no, trn_date, trn_amount,
				trn_disc, trn_vat, trn_netamount, trn_ptype, trn_rcvamount, trn_ref, trn_desc, ent_id)
				VALUES('".$hslno."','".$_SESSION['zid']."','2','0','".$_POST['dcus_cod']."','".$_POST['trn_mobile_no']."','".$_POST['trn_date']."','".$_POST['trn_sub_total']."',
				'".$_POST['trn_discount']."','".$_POST['trn_vat_amnt']."','".$_POST['trn_net_pay']."','".$_POST['trn_ptype']."',
				'".$_POST['trn_cash']."','".$_POST['cus_mob']."','POS sales','".$_SESSION['user_id']."')";
				...
				if(!@mysql_errno()){
					...
					if(!@mysql_errno()){
						...
						$Acc_sts=$accounts->posToAccounts($data,'add');
						if($Acc_sts===true){
							$_SESSION['pos_slno']=$hslno;
							$title="Done!";
				...
				trn_mobile_no='".$_POST['trn_mobile_no']."',
				...
				WHERE slno='".$_SESSION['pos_slno']."'";
				$cmncls->sqlqry($hupdt_sql);
				if(!@mysql_errno()){
					$cmncls->delete('inop_details','hslno',$_SESSION['pos_slno']);
					$sdetail_sql="INSERT INTO inop_details(hslno, zid, item_code, item_qty, sale_rate,buy_rate, disc_rate, vat_rate, ent_id) VALUES ";
					foreach($_SESSION['item_row'] as $item_info){
						$cost_goods+=round(($item_info['item_brate']*$item_info['item_qty']),2);
						$sdetail_sqli.="('".$_SESSION['pos_slno']."','".$_SESSION['zid']."','".$item_info['item_code']."','".$item_info['item_qty']."','".$item_info['item_srate']."','".$item_info['item_brate']."','".$item_info['item_discount']."','".$item_info['item_vatrate']."','".$_SESSION['user_id']."'),";
					}
					if($sdetail_sqli!=NULL){
						$sdetail_sql.=substr($sdetail_sqli, 0, -1);
						$cmncls->sqlqry($sdetail_sql);
					...
					if(!@mysql_errno()){
						...
						$Acc_sts=$accounts->posToAccounts($data,'update');
						if($Acc_sts===true){
							$title="Done!";
							$text="Successfully Saved!";
							$type="success";
